...
extern char buff[256];		// used in help.c, encoders.c
...
{
	const int e=0;
	byte Q2 = (ENCODER_1Q2_PIN & ENCODER_1Q2_MASK)>0;

	if (Q2==1) Encoders[e].Direction = CW;
	else       Encoders[e].Direction = CCW;
	
	if (Encoders[0].Direction == CW)
		Encoders[0].Count++;		// never reset.  Holds _THE_ count from min stop.
	else 
		Encoders[0].Count--;
...
{
	const int e=1;
	byte Q2 = (ENCODER_2Q2_PIN & ENCODER_2Q2_MASK)>0;
...
{
	const int e=2;
	byte Q2 = (ENCODER_3Q2_PIN & ENCODER_3Q2_MASK)>0;
...
ISR ( INT3_vect )
{
	led_on(WARNING_LED);
	const int e=3;
	byte Q2 = (ENCODER_4Q2_PIN & ENCODER_4Q2_MASK)>0;
...
ISR ( INT4_vect )
{
	led_on(WARNING_LED);
	const int e=4;
	byte Q2 = (ENCODER_5Q2_PIN & ENCODER_5Q2_MASK)>0;
...
void encoder_init()
{
	...
	ENCODER_G1_PORT	|= (ENCODER_G1_MASK);		// pull-ups - to prevent spurious interrupts (motor noise)

	ENCODER_G2_DDR	&= ~(ENCODER_G2_MASK);
	ENCODER_G2_PORT	|= (ENCODER_G2_MASK);		// pull-ups - encoders will idle low when connected.
...
/* Returns the value based on Configuration!  */
uint32_t get_position( char mLetter )
{
	int index = get_letter_index( mLetter );
	if (FiveMotorConfigData.use_encoder[index])
		return Encoders[index].Count;
	else
		return PotSample[index];
}

void print_encoder_selections()
{
	sprintf(buff, "\r\nUse_encoder: \r\n" );
	send_message( buff );
	for (int m=0; m<NUM_MOTORS; m++)
	{
		if (FiveMotorConfigData.use_encoder[m]>0)
			sprintf(buff, "%c - Encoder\r\n", motor_letters[m] );
		else
			sprintf(buff, "%c - Potentiometer\r\n", motor_letters[m] );
		send_message(buff);
	}
}
