...
{
	int v_index = get_letter_index('v');
	int w_index = get_letter_index('w');
	...
	// Actuate : 
	start_pid_control('v', mot_states[v_index].destination );
	start_pid_control('w', mot_states[w_index].destination );
}
...
/*	float circumference 		= M_PI * FiveMotorConfigData.wheel_circumference;
	...
	if ( mot_states[v_index].direction == FALSE )
		mot_states[v_index].duty_fraction = -fraction;
	if ( mot_states[w_index].direction == FALSE )
